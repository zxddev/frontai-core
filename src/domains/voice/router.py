...
完整流程: 用户语音 → ASR识别 → LLM生成回复 → TTS合成 → 返回音频
...
3. 服务端返回 JSON 消息:
   ...
   - {"type": "ai_response", "text": "...", "audio": "base64..."}  AI回复（含音频）
...
class VoiceChatManager:
    ...
    async def _handle_audio(self, session: VoiceSession, audio_data: bytes) -> None:
        ...
        logger.debug(f"收到音频数据: {len(audio_data)} bytes, 总计: {len(session.audio_buffer)} bytes")
    ...
    async def _handle_control(self, session: VoiceSession, message: str) -> None:
        ...
        if msg_type == "start":
            ...
            logger.info(f"开始录音: {session.session_id}")
            await self._send_json(session.websocket, {"type": "recording"})

        elif msg_type == "stop":
            session.is_recording = False
            logger.info(f"停止录音: {session.session_id}, 音频大小: {len(session.audio_buffer)} bytes")
